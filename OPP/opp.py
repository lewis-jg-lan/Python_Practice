...
def initData():
    ...
    for i in range(0, len(raw_data_col)):
        if i % len(log_data) == 0:
            final_data_col.append(raw_data_col[i:i + len(log_data)])
            for j in range(10,len(final_data_col)):
                if final_data_col[j][7].isdecimal() == False:
                    del final_data_col[j]

    global col_count, row_count
    col_count = len(final_data_col)
    row_count = len(log_data)

    for i in range(col_count):
        print(final_data_col[i])
...
def toStampTime(tt):
    if tt != '':
        import time
        timeArray = time.strptime(tt, "%Y/%m/%d %H:%M")
...
#得到平均值
def getMeanValue(testValue):
    if len(testValue) > 0:
        Sum = 0.0
        for j in range(0,len(testValue)):
            Sum += testValue[j]
        meanValue = Sum/len(testValue)
        meanValue = float('%.6f' % meanValue)
        return meanValue

#得到方差
def getStdValue(testValue):
    if len(testValue) > 0:
        Sum = 0.0
        for j in range(0, len(testValue)):
            Sum += testValue[j]
        ...
        S1 = 0.0
        for j in range(0, len(testValue)):
            S1 += (testValue[j] - meanValue)**2

        stdValue = S1/len(testValue)
        stdValue = float('%.6f' % stdValue)
        return  stdValue
...
def drawDistributionChart():
    ...
    for i in range(10,col_count):
        ...
        meanTestValue = getMeanValue(testValue)
        print('meanValue =',meanTestValue)
        stdTestValue = getStdValue(testValue)
        print('stdValue =',stdTestValue)
        ...
        strString.append('Mean Value : ' +str(meanTestValue))
        strString.append('Std Value : ' +str(stdTestValue))
        #写text到图片
